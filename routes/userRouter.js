...
import { findUser, createUserToDb } from "../models/usersModel.js";
...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (user.length === 0) {
      return res
        .status(404)
        .json({ message: "fel användarnamn eller lösenord 111" });
    ...
    if (checkIfMatch) {
      ...
      res.status(401).json({ message: "fel användarnamn eller lösenord 222" });
    }
  } catch (err) {
    console.log(err);
